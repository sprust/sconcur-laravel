...
|---|---|---|
| PHP | 8.4, NTS | |
| `ext-msgpack` | 3.0.1 | весь трафик через границу PHP↔Go; жёсткое требование `sconcur/sconcur`, его проверяет composer |
| расширение `sconcur` | 0.11.0 | ровно версия `sconcur/sconcur`, ставится отдельно (шаг 2) |
...
поэтому `sconcur/sconcur` закреплён точно (`0.11.0`), а не кареткой, а расширение должно
...
- **Порядок колонок в строке не гарантирован.** PDO отдаёт их в порядке `SELECT`, здесь
  строка едет через границу PHP↔Go как msgpack-map, а у map порядок ключей не сохраняется
...
| Блок | Что показывает |
|---|---|
| Master telemetry | панель мастера как есть: воркеры по группам, сколько запросов и сообщений в работе прямо сейчас, память, горутины. Обновляется раз в секунду |
