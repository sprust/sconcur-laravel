...
Разобранные примеры на всё это — публичные, приватные и presence-каналы, `toOthers()`,
клиентские события, вещание из задания и из задачи, события самого пула и что проверять,
когда ничего не приходит — в
[docs/websocket.ru.md](docs/websocket.ru.md#примеры).

Echo не обязателен, и фронт на Blade тоже: SPA на своём origin работает так же, клиент
может говорить на протоколе голым `WebSocket`, а вещать в пул умеет любой сервис,
способный положить сообщение в fanout-обмен. За Laravel остаётся выдача подписи на
канал.
