...
Проектные решения и их обоснование — в `.ai/plans/websocket/`.
...
| `4200` | внутренняя ошибка, переподключение допустимо |
...
через fanout-обмен RabbitMQ, а `Channel::consume()` из библиотеки — генератор,
...
- первое соединение поднимает его через `Scheduler::spawn`;
