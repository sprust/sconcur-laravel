...
Пул консьюмеров и пул задач публикуют в ту же шину тем же драйвером.
...
Прокси перед пулом нужен отдельный `location` под Upgrade. Поднятое соединение живёт
долго, поэтому таймауты обычного блока рвали бы каждого клиента раз в минуту:

```nginx
# `map` живёт на уровне http, вне server-блока: «upgrade», пока клиент его просит, и
# «close» в остальных случаях, чтобы обычному запросу не предлагали сменить протокол.
...
`location` обязан совпадать с `SCONCUR_WS_PATH_PREFIX`; ключ идёт в пути следом.
...
Чего проверка не покрывает. `--host` через прокси захватывает и его блок с Upgrade, а на
собственный порт пула — не захватывает: проверка, которая там проходит, пока браузеры
отваливаются, как раз и указывает на прокси. Канал публичный, поэтому авторизация не
затрагивается; чтобы покрыть и подпись, надо взять её у `/broadcasting/auth` и положить в
`pusher:subscribe`. А соединение попадает на того воркера, кого выдало ядро, поэтому пул
больше одного воркера покрывается запуском проверки столько раз, сколько воркеров, со
сбором пидов.
...
Издающая сторона держит одно соединение и один канал на всю жизнь процесса: процесс вещает
вспышками, и канал на сообщение означал бы дозвон на сообщение. Держится этот канал не
безусловно. Расширение собирает канал без потребителей, на котором полчаса не было ни одной
команды, и этой стороне не говорит ничего — ручка по-прежнему выглядит открытой, а
публикация, завершающая долгую тишину, падает на канале, которого уже нет, с текстом
`No channel available`. Поэтому шина отдаёт канал, простоявший десять минут, до того как
выдать его, а не после провала, а публикация, упавшая на удержанном канале, повторяется один
раз на канале, который на месте. Публикация, открывшая канал сама, не повторяется: то, на чём
она упала, — ответ брокера, и вторая попытка придёт к тому же самому.

Повторённая публикация может дойти до брокера дважды: команда, упавшая по дороге, всё же
могла быть выполнена. Значит, вещание доставляется не ровно один раз, а как минимум один раз.
Для уведомления это дешёвая сторона размена: другая его сторона — та самая тишина, ради
которой повтор и сделан.
...
Группа `ws` в `config('sconcur.master.groups')` и секция `config('sconcur.ws')` — обе
перечислены в [configuration.ru.md](configuration.ru.md#группа-ws).

Два значения нельзя выставить «как у группы `http`». `handlerTimeoutMs` жёстко равен нулю,
потому что здесь это дедлайн на всю жизнь соединения, а не на кадр, и что угодно выше нуля
рвёт всех клиентов по таймеру. `idleTimeoutMs` по умолчанию ноль, потому что молчание
клиента нормально — живость держит `pingIntervalMs`.
