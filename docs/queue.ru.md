...
Watchdog мастера — другое, и он включён. Джоба, держащая PHP-поток в нативном вызове —
запрос через PDO, `curl`, `sleep()`, — дольше `watchdogTimeoutMs` (60 с по умолчанию,
`SCONCUR_HTTP_WATCHDOG_TIMEOUT_MS`), оставляет воркер неспособным обслужить что-либо, его
остальные обработчики тоже, и мастер его заменяет: `SIGTERM`, затем `SIGKILL` через
`shutdownTimeoutMs` группы. Сообщение, не подтверждённое к смерти воркера, будет
доставлено снова — в том числе заблокировавшая джоба, если к тому времени не закончилась,
и тогда она так же заблокирует следующий воркер. Джоба, ждущая через SConcur
(`sconcur_mysql`, HTTP-клиент фичи, `Sleeper`), вместо этого приостанавливает свою
корутину и за зависание не принимается. Джобе, которой законно нужно так долго блокировать
нативно, нужен порог выше этого времени или `0` для группы `rabbitmq`.
