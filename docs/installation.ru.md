...
| расширение `sconcur` | 0.14.0 | ровно версия `sconcur/sconcur`, ставится отдельно (шаг 2) |
...
поэтому `sconcur/sconcur` закреплён точно (`0.14.0`), а не кареткой, а расширение должно
