...
| the `sconcur` extension | 0.13.0 | exactly the `sconcur/sconcur` version; installed separately (step 2) |
...
`sconcur/sconcur` is pinned exactly (`0.13.0`) rather than with a caret, and the
