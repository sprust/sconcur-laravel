...
`workbench/` живёт под `orchestra/testbench` и принадлежит тестам. И у него, и у `demo/`
есть свой `bootstrap/app.php`, возвращающий `AsyncApplication`: testbench подключает
workbench-овский вместо того, чтобы собирать приложение самому, поэтому тесты идут на том
же классе, что и приложение. `demo/` остаётся отдельным, потому что это настоящий скелет,
который обслуживает мастер, — со своими `public/`, `artisan`, роутами и вьюхами.
