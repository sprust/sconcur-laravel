...
Ещё четыре ключа того же массива настраивают пул. Их читает `Connector` и передаёт
расширению; чего в массиве нет, у того остаётся дефолт расширения.

| Ключ | Дефолт | Назначение |
|---|---|---|
| `timeout_ms` | `30000` (расширения) | предел на один statement; для курсора — на всю его жизнь |
| `max_open_conns` | `20` (пакета) | размер пула в расширении; `0` — это не «без лимита», а его собственные 32 |
| `max_idle_conns` | — | принимается и не применяется: пул держит все простаивающие соединения до предела. Ключом пула он при этом остаётся, поэтому два значения — это два пула |
| `conn_max_lifetime_ms` | — | время жизни соединения; отсутствует — без предела |

`max_open_conns` — единственный дефолт, который пакет переопределяет, и пул с
ограничением тут не перестраховка: каждый одновременный statement берёт своё соединение,
поэтому безлимитный пул при фан-ауте упирается в `max_connections` сервера (ошибка MySQL
1040).
...
появления первого фибера, досталась бы каждому запросу, сообщению и задаче разом. Первый
уровень — настоящий `BEGIN` фичи, выше — `SAVEPOINT`, как и на PDO.
...
транзакции подходят `select()`/`get()`, которые вычитывают выборку целиком.
...
корутины у него один собственный менеджер, и поведение неотличимо от штатного. Без него
один синглтон на процесс ключевал бы записи именем соединения, а не тем, кто открыл
транзакцию, и коммит
одной корутины выполнял бы `afterCommit` соседней при ещё открытой у той транзакции.
`Model::saveOrFail()` — это транзакция, так что обычное создание модели
уже туда попадает.
...
- `getPdo()`/`getReadPdo()` бросают исключение: объекта PDO нет. Код, которому нужен
  PDO, работает через соединение `mysql`.
...
- **Порядок колонок в строке не гарантирован.** PDO отдаёт их в порядке `SELECT`, здесь
  строка едет через границу PHP↔расширение как msgpack-map, а у map порядок ключей не
  сохраняется — так что подряд идущие одинаковые запросы дают разный порядок полей.
  По имени всё
