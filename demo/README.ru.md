...
## Проверка пула без браузера

Страница — обычный способ посмотреть на пул, но по ней нельзя прочитать код возврата.
`demo/bin/ws-check.php` проходит тот же путь снаружи и отчитывается по каждому шагу:

```bash
make ws-check          # пачка из пяти
make ws-check c=50     # самая большая пачка, которую умеет панель
```
...
| Шаг | Что доказывает |
|---|---|
| handshake | nginx пропускает Upgrade, а ws-пул отвечает `pusher:connection_established` |
| the socket id names a worker | id — это пид воркера и счётчик, поэтому по нему видно, какой ws-воркер взял соединение |
| ping | соединение живо в обе стороны, а не просто открыто |
| subscribe | канал принят — для публичного канала без обращения к `/broadcasting/auth` |
| publish over http | http-воркер принял пачку; его пид отличается от пида ws-воркера, и эту границу как раз пересекает шина |
| the whole burst arrives | вернулись все сообщения, пронумерованные с единицы, ничего не пропало и не пришло дважды |

Скрипт запускается в контейнере workers, где лежит расширение с ws-клиентом, и выходит с
ненулевым кодом на первом отказе. Отвергнутый Upgrade показывается как неудавшееся
рукопожатие с причиной в той же строке, а не стектрейсом: например, неверный
`SCONCUR_WS_APP_KEY` даёт `Invalid status code: 404`, потому что ключ лежит в пути, и
несовпавший путь отвергается до того, как о нём узнает PHP.

`make sconcur-status` и `make ext-status` отвечают на два вопроса, которые идут до всего
этого: поднята ли вообще группа `ws` и совпадает ли загруженное расширение с пакетом.
