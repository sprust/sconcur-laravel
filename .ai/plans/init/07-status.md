...
- Решение по `tasks:restart` (см. «Найдено по дороге»). Тест
  `TaskPoolStateTest::aRelaunchDoesNotReactivateAStoppedTask` — характеризация, а не
  одобрение: он и есть то место, которое надо будет поправить.
