...
## Совместимость фасада с phpredis

Решение мейнтейнера (2026-09-16): `REDIS_CLIENT=sconcur` должен быть полной заменой phpredis
для кода, написанного под `Redis::`. Эталон — `PhpRedisConnection` Laravel, а не predis:
phpredis — клиент Laravel по умолчанию и самый распространённый.

- Фасад — тонкая прослойка: кроме ~25 переопределений Laravel вызов уходит в родной метод
  phpredis, поэтому контракт — сигнатуры и форма ответов phpredis.
- phpredis установлен только в `scl-php` (`pie install phpredis/phpredis:6.3.0`), для
  `PhpRedisParityTest`: одинаковые вызовы через `PhpRedisConnection` и через наш
  `Connection`, `assertSame`. Проверено мутацией: без приведения ответов падает 75 кейсов.
- `Connection` повторяет переопределения Laravel; `PhpRedisArguments` читает сигнатуры phpredis
  с массивами опций; `PhpRedisReplies` приводит ответы (статус → `true`, nil → `false`,
  0/1 → `bool`, float, `type` → константа, карты `hgetall`/`WITHSCORES`/`hmget`, разбор
  `info`, ошибка в пакете → `false`). `CommandBatch` делает то же для пайплайна/транзакции.
- Осознанные отличия: отвергнутая команда бросает исключение (phpredis — `false`);
  `scan`-семейство отвечает `[курсор, элементы]` (у phpredis курсор по ссылке); статус из
  Lua-скрипта остаётся `'OK'` — фича не различает статус и bulk-строку.
- Замечено при сверке и не повторяется: `set(k, v, 'GET')` через переопределение Laravel
  отвечает `true` (сбой в самом Laravel), `zRange(... ['byscore' => true])` у phpredis 6.3.0
  вернул не тот диапазон.
- Штатный драйвер очереди `redis` работает без изменений (`QueueTest`); `bulk()` не атомарный.
