...
## Правки по третьему ревью

- Копирование каталога обрезало назначение (фича открывает каталог без ошибки и падает
  только на чтении). Теперь всё, что не обычный файл, — нативно: источник копирования,
  назначение записи/копирования, чтение и хеш (`LocalPaths::isRegularFile()`,
  `isRegularOrMissing()`); у FIFO и `/proc` фича читала бы по нулевому размеру.
- `open_basedir` фича не видит — при заданном `open_basedir` всё нативно (`FilesFeatureCall`).
- Вызов на фиче в `finally` сворачиваемой корутины приостанавливался навсегда.
  `Coroutine::isActive()` теперь требует fiber расширения (`State::isAsyncFiber`) и
  `Scheduler::canAwait()`; это же чинит `CooperativeSleep` и fiber чужих библиотек.
- `writeStream` крутился на неблокирующем потоке — выход на первом пустом чтении, как у
  `file_put_contents()`; писатель закрывается и при ошибке фичи посреди потока.
- `move` убран из обоих классов целиком: внутри ФС `rename(2)` выигрывать нечего, между ФС и
  через bind mount фича ведёт себя иначе. `crossesDevices()` и `CrossDeviceMoveTest` удалены.
- `File::replace`: права не в виде простых битов (setuid/setgid/sticky, биты типа, строка) —
  нативно. `lastModified` до 1970 — нативно.
- Пул задач: `sconcur:tasks:stop` во время остановки по сигналу снимает перезапуск.
- Одновременный доступ из корутин — та же гонка, что между процессами (решение
  мейнтейнера): не чиним, описано в документации.
- Тесты: `CoroutineUnwindTest`, `OpenBasedirTest` (отдельный процесс PHP), новые parity-кейсы
  (каталог как источник и назначение, режимы `replace`, неблокирующий поток, mtime до 1970),
  `StatCacheTest` снимает состояние внутри корутины, пороги простоя относительные.
